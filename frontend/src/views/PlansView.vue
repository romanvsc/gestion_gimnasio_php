    ...
    <div class="page-container flex flex-col gap-5">
      <header class="section-header">
        <div>
          ...
          <p class="mt-2 text-sm text-ink-500">Precios, duración y estado de las cuotas del gimnasio</p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
        <article class="metric-tile border-t-4 border-t-forest-0">
          <div class="relative z-10">
            <p class="text-xs font-extrabold uppercase text-ink-500">Planes activos</p>
            <p class="mt-3 font-heading text-4xl font-bold text-ink-0">{{ pad(activePlans.length) }}</p>
            <p class="mt-2 text-sm font-semibold text-forest-900">Disponibles para socios</p>
          </div>
        </article>
        <article class="metric-tile border-t-4 border-t-slate-0">
          <div class="relative z-10">
            ...
            <p class="mt-3 font-heading text-4xl font-bold text-ink-0">{{ compactCurrency(averagePrice) }}</p>
            <p class="mt-2 text-sm font-semibold text-slate-800">Entre planes activos</p>
          </div>
        </article>
        <article class="metric-tile border-t-4 border-t-timber-0">
          <div class="relative z-10">
            <p class="text-xs font-extrabold uppercase text-ink-500">Duración media</p>
            <p class="mt-3 font-heading text-4xl font-bold text-ink-0">{{ averageDuration }}d</p>
            <p class="mt-2 text-sm font-semibold text-timber-800">Ciclo de cuota</p>
          </div>
        </article>
      </div>
      ...
      <form v-if="editingPlan" class="panel-card" @submit.prevent="savePlan">
        <div class="mb-4 flex items-center justify-between gap-3">
          <div>
            <p class="text-xs font-extrabold uppercase text-ink-500">Editor de plan</p>
            <h2 class="mt-1 font-heading text-3xl font-bold uppercase text-ink-0">{{ planForm.id ? 'Editar membresía' : 'Nueva membresía' }}</h2>
          </div>
        ...
        <div class="grid gap-3 md:grid-cols-4">
          <div class="md:col-span-2">
            <label for="plan-name" class="mb-1.5 block text-xs font-semibold uppercase text-ink-500">Nombre</label>
            <input id="plan-name" v-model="planForm.name" class="input-base" required placeholder="Mensual libre" />
          </div>
          <div>
            <label for="plan-price" class="mb-1.5 block text-xs font-semibold uppercase text-ink-500">Precio</label>
            <input id="plan-price" v-model.number="planForm.price" type="number" min="1" step="0.01" class="input-base" required />
          </div>
          <div>
            <label for="plan-club-price" class="mb-1.5 block text-xs font-semibold uppercase text-ink-500">Precio socio club</label>
          ...
          <div>
            <label for="plan-duration" class="mb-1.5 block text-xs font-semibold uppercase text-ink-500">Días</label>
            <input id="plan-duration" v-model.number="planForm.duration_days" type="number" min="1" class="input-base" required />
          </div>
          <div>
            <label for="plan-status" class="mb-1.5 block text-xs font-semibold uppercase text-ink-500">Estado</label>
          ...
          <div class="md:col-span-3">
            <label for="plan-description" class="mb-1.5 block text-xs font-semibold uppercase text-ink-500">Descripción</label>
            <input id="plan-description" v-model="planForm.description" class="input-base" placeholder="Acceso libre, clases incluidas, promo..." />
          </div>
        </div>

        <p v-if="planError" class="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800" role="alert">{{ planError }}</p>
      ...
      <section v-if="settingsStore.loading" class="grid gap-3 lg:grid-cols-3">
        <div v-for="i in 3" :key="i" class="glass-card h-44 animate-pulse bg-paper-100"></div>
      </section>
      ...
      <section v-else-if="settingsStore.plans.length" class="grid gap-3 lg:grid-cols-3">
        ...
        >
          <div class="h-1.5" :class="plan.status === 'active' ? 'bg-forest-0' : 'bg-paper-300'"></div>
          <div class="p-5">
            <div class="flex items-start justify-between gap-3">
              <div>
                <p class="text-xs font-extrabold uppercase text-ink-500">Plan</p>
                <h2 class="mt-1 font-heading text-3xl font-bold uppercase text-ink-0">{{ plan.name }}</h2>
              </div>
              <span :class="plan.status === 'active' ? 'badge-active' : 'badge-inactive'">
                {{ plan.status === 'active' ? 'Activo' : 'Inactivo' }}
              </span>
            </div>
            <p class="mt-4 font-heading text-4xl font-bold text-forest-900">{{ formatCurrency(plan.price) }}</p>
            <p v-if="plan.club_member_price" class="mt-1 text-sm font-semibold text-slate-800">
              Socio club {{ formatCurrency(plan.club_member_price) }}
            </p>
            <p class="mt-1 text-sm font-semibold text-ink-500">{{ plan.duration_days }} días de vigencia</p>
            <p class="mt-4 min-h-10 text-sm leading-5 text-ink-700">{{ plan.description || 'Sin descripción cargada.' }}</p>
            <div class="mt-5 flex gap-2 border-t border-paper-300 pt-4">
      ...
      <section v-else class="glass-card px-6 py-12 text-center">
        <IconPlans class="mx-auto h-12 w-12 text-ink-500" />
        <p class="mt-4 text-sm font-medium text-ink-500">Todavía no hay planes creados</p>
