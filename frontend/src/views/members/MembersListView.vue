    ...
    <div class="page-container flex flex-col gap-5">
      <header class="section-header">
        <div>
          ...
          <p class="mt-2 text-sm text-content-secondary">{{ store.meta.total }} fichas encontradas</p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <button class="metric-tile border-t-4 border-t-status-success text-left" @click="applyQuickFilter('active', '')">
          <div class="relative z-10">
            <p class="text-xs font-extrabold uppercase text-content-secondary">Socios activos</p>
            <p class="mt-3 font-heading text-4xl font-bold text-content">{{ pad(stats.active) }}</p>
            <p class="mt-2 text-sm font-semibold text-status-success">Base operativa</p>
          </div>
        </button>
        <button class="metric-tile border-t-4 border-t-status-success text-left" @click="applyQuickFilter('active', 'paid')">
          <div class="relative z-10">
            <p class="text-xs font-extrabold uppercase text-content-secondary">Cuota al día</p>
            <p class="mt-3 font-heading text-4xl font-bold text-content">{{ pad(stats.paid) }}</p>
            <p class="mt-2 text-sm font-semibold text-status-success">Vigencia activa</p>
          </div>
        </button>
        <button class="metric-tile border-t-4 border-t-status-danger text-left" @click="applyQuickFilter('active', 'debt')">
          <div class="relative z-10">
            <p class="text-xs font-extrabold uppercase text-content-secondary">En mora</p>
            <p class="mt-3 font-heading text-4xl font-bold text-content">{{ pad(stats.debt) }}</p>
            <p class="mt-2 text-sm font-semibold text-status-danger">Requieren seguimiento</p>
          </div>
        </button>
      </div>
      ...
      <section class="panel-card">
        <div class="flex flex-col gap-3 xl:flex-row xl:items-center">
          <div class="relative flex-1">
            ...
            <IconSearch class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-content-secondary" />
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :class="statusFilter === filter.value ? 'bg-brand text-white' : 'border border-border-strong bg-surface-muted text-content-strong hover:bg-surface-elevated'"
            >
              {{ filter.label }}
            </button>
          </div>
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              class="rounded-lg px-3 py-2 text-sm font-semibold transition"
              :class="quotaFilter === filter.value ? 'bg-brand text-white' : 'border border-border-strong bg-surface-muted text-content-strong hover:bg-surface-elevated'"
            >
              {{ filter.label }}
            </button>
          </div>

          <label for="members-page-size" class="flex items-center gap-2 rounded-lg border border-border-strong bg-surface-muted px-3 py-2 text-sm text-content-secondary">
            <span>Por página</span>
            <select id="members-page-size" v-model.number="pageSize" @change="changePageSize" class="bg-transparent text-content focus:outline-none">
      ...
      <div v-if="store.loading" class="space-y-2">
        <div v-for="i in 8" :key="i" class="surface-card h-20 animate-pulse bg-surface-muted"></div>
      </div>

      <section v-else-if="store.members.length" class="surface-card overflow-hidden">
          ...
          <table class="w-full min-w-[920px] text-sm">
            ...
            <tbody>
              <tr
                ...
                :key="member.id"
                class="border-b border-border-strong transition hover:bg-surface-muted"
                @click="$router.push(`/members/${member.id}`)"
              >
                <td class="table-body-cell cursor-pointer">
                  <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-surface-strong">
                      <span class="font-heading text-sm font-bold text-content-secondary">{{ initials(member) }}</span>
                    </div>
                    <div>
                      <p class="font-semibold text-content">{{ member.first_name }} {{ member.last_name }}</p>
                      <p class="text-xs text-content-secondary">DNI {{ member.dni || '-' }}</p>
                    </div>
                  </div>
                </td>
                <td class="table-body-cell text-content-secondary">
                  <p>{{ member.phone || 'Sin teléfono' }}</p>
                  <p class="text-xs">{{ member.email || 'Sin email' }}</p>
                </td>
                <td class="table-body-cell">
                  <p class="font-semibold text-content">{{ member.plan_name || 'Sin plan' }}</p>
                  <p class="text-xs text-content-secondary">{{ member.plan_price ? formatCurrency(member.plan_price) : 'Sin precio' }}</p>
                </td>
                <td class="table-body-cell">
                ...
                <td class="table-body-cell">
                  <div class="flex justify-end gap-2">
                    <span class="action-tooltip-wrap">
                      <button
                        class="icon-action text-status-success"
                    ...
                    <span class="action-tooltip-wrap">
                      <button
                        class="icon-action text-content-secondary"
        ...
        <div class="divide-y divide-border-strong lg:hidden">
          <article
            v-for="member in store.members"
            :key="member.id"
            class="p-4"
          ...
          >
            <div class="flex items-start gap-3">
              <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-surface-strong">
                <span class="font-heading text-base font-bold text-content-secondary">{{ initials(member) }}</span>
              </div>
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-1 text-xs text-content-secondary">{{ member.plan_name || 'Sin plan' }} - {{ member.phone || member.dni || 'Sin contacto' }}</p>
      ...
      <section v-else class="surface-card px-6 py-12 text-center">
        <IconMembers class="mx-auto h-12 w-12 text-content-secondary" />
        <p class="mt-4 text-sm font-medium text-content-secondary">No se encontraron socios</p>
      ...
      <div v-if="store.members.length" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-content-secondary">Página {{ store.meta.page }} de {{ store.meta.pages || 1 }} - {{ store.meta.total }} socios</p>
