...
CREATE TABLE IF NOT EXISTS `payment_plans` (
  ...
  `club_member_price` DECIMAL(10,2) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `members` (
  ...
  `medical_certificate_valid_until` DATE DEFAULT NULL,
  ...
  KEY `idx_members_medical_certificate` (`company_id`, `medical_certificate_valid_until`),
...
CREATE TABLE IF NOT EXISTS `payments` (
  ...
  `legacy_method_name` VARCHAR(80)            DEFAULT NULL,
  `notes`           TEXT                     DEFAULT NULL,
  ...
  KEY `idx_payments_period`       (`company_id`, `period_start`, `period_end`),
...
CREATE TABLE IF NOT EXISTS `checkins` (
  ...
  KEY `idx_checkins_access_allowed` (`company_id`, `access_allowed`, `checkin_at`),
